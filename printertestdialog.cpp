...
PrinterTestDialog::~PrinterTestDialog()
{
    delete ui;
}

void PrinterTestDialog::printerTest()
{
    qDebug() << "on_printButton_clicked";

    fd = ::open("/dev/printer_device_node", O_RDWR);
    ...
    for (i = 0; i < width; i++) {
        for (j = 0; j < height; j++) {
            ...
            sum = integralImageSumArr[y2 * width + x2] - integralImageSumArr[y1 * width + x2] - integralImageSumArr[y2 * width + x1] + integralImageSumArr[y1 * width + x1];
            if ((long)(matriPicDealGlobal[j][i] * count) < (long)(sum * (100 - T) / 100)) {
...
bool PrinterTestDialog::toGray()
{
    ...
    if (image->width() != PICW || image->height() != PICH) {
        qDebug() << "toGray size error image size : " << image->width() << "  " << image->height();
        return false;
    }
    qDebug() << "toGray image size : " << image->width() << "  " << image->height();
    ...
    for (int y = 0; y < image->height(); y++) {
        ...
        for (int x = 0; x < image->width(); x++) {
            pixValT = qRed(line[x]) * 0.114 + qGreen(line[x]) * 0.587 + qBlue(line[x]) * 0.299;
