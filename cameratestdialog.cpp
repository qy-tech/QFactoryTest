...
void CameraTestDialog::openCamera()
{
    qDebug("vo_test0");
    ...
    stVoAttr.stDispRect.u32Width = 720; //区域的宽度
    ...
    VENC_ROTATION_E enRotation = VENC_ROTATION_0; //VENC_ROTATION_90 VENC_ROTATION_180

    VENC_CHN_ATTR_S venc_chn_attr; //VENC通道属性结构体。
    memset(&venc_chn_attr, 0, sizeof(venc_chn_attr));
    venc_chn_attr.stVencAttr.enType = RK_CODEC_TYPE_JPEG; //VENC_ATTR_S stVencAttr:编码器属性。enType:编码协议类型。
    venc_chn_attr.stVencAttr.imageType = enPixFmt; // 输入图像类型
    venc_chn_attr.stVencAttr.u32PicWidth = u32SrcWidth; //编码图像宽度。以像素为单位。
    venc_chn_attr.stVencAttr.u32PicHeight = u32SrcHeight; //编码图像高度。以像素为单位。
    venc_chn_attr.stVencAttr.u32VirWidth = u32SrcWidth; //stride宽度，通常与buffer_width相同。若u32VirWidth大于buffer宽度，则必须满足16对齐。
    venc_chn_attr.stVencAttr.u32VirHeight = u32SrcHeight; //stride高度，通常与buffer_height相同。若u32VirHeight大于buffer高度，则必须满足16对齐。
    venc_chn_attr.stVencAttr.stAttrJpege.u32ZoomWidth = u32DstWidth; //某种协议的编码器属性。这里使用stAttrJpege: attributes of jpeg.  u32ZoomWidth:缩放的指定宽度。
    venc_chn_attr.stVencAttr.stAttrJpege.u32ZoomHeight = u32DstHeight; //缩放的指定高度。
    venc_chn_attr.stVencAttr.stAttrJpege.u32ZoomVirWidth = u32DstWidth; //缩放的虚拟宽度。
    venc_chn_attr.stVencAttr.stAttrJpege.u32ZoomVirHeight = u32DstHeight; //缩放的虚拟高度。
    ...
    stEncChn.s32ChnId = 0;
    ret = RK_MPI_SYS_RegisterOutCb(&stEncChn, this->video_packet_callback); //注册数据输出回调。
    if (ret) {
...
void CameraTestDialog::closeEvent(QCloseEvent*)
{
    qDebug("CameraTestDialog close");
    closeCamera();
}
